...
<script src="http://localhost/miniproject/assets/js/myscript.js" type="text/javascript"></script>
<!-- END JAVASCRIPTS -->
<script>
	<?php if ($this->session->flashdata('daftar_bel')): ?> 
		Swal.fire({
			icon : 'info',
			title : 'Oops...',
			text : 'Anda telah mendaftar kegiatan ini'
		});
	<?php endif; ?>
</script>
<script>
	<?php if ($this->session->flashdata('daftar_kom')): ?> 
		Swal.fire({
			icon : 'info',
			title : 'Oops...',
			text : 'Anda telah mendaftar kegiatan ini'
...
<script>
	<?php if ($this->session->flashdata('daftar_web')): ?> 
		Swal.fire({
			icon : 'info',
			title : 'Oops...',
			text : 'Anda telah mendaftar webinar ini'
		});
	<?php endif; ?>
</script>
<script>
	<?php if ($this->session->flashdata('error_daftarbel')): ?> 
		Swal.fire({
			icon : 'error',
			title : 'Oops...',
			text : 'Waktu pendaftaran kegiatan sudah ditutup!'
		});
	<?php endif; ?>
</script>
<script>
	<?php if ($this->session->flashdata('error_daftarkom')): ?> 
		Swal.fire({
			icon : 'error',
			title : 'Oops...',
			text : 'Waktu pendaftaran kegiatan sudah ditutup!'
...
<script>
	<?php if ($this->session->flashdata('error_daftarweb')): ?> 
		Swal.fire({
			icon : 'error',
			title : 'Oops...',
			text : 'Waktu pendaftaran webinar sudah ditutup!'
		});
	<?php endif; ?>
</script>
<script>
	<?php if ($this->session->flashdata('error_gantipass')): ?> 
		Swal.fire({
			icon : 'error',
			title : 'Gagal',
...
<script>
	<?php if ($this->session->flashdata('error_gantipass2')): ?> 
		Swal.fire({
			icon : 'error',
			title : 'Gagal',
...
<script>
	<?php if ($this->session->flashdata('error_file')): ?> 
		Swal.fire({
			icon : 'error',
			title : 'Gagal',
			text : 'Ukuran file yang di upload terlalu besar'
		});
	<?php endif; ?>
</script>
</body>
